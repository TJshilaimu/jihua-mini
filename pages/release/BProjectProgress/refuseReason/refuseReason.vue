	...
	<view class="personalIntroduction">
		<view class="textarea">
			...
			<view class="textRight">
				<text :class="colorText?'textRed':'textColor'" >{{talVal}}</text>/100		
			</view>
		</view>
		<button class="btnTrue" :class="[content?'':'disabled']" @click="btnTrue">确定</button>
...
<script>
	export default {
		...
		onLoad(ops) {
			//上一个页面传来的参数,获取并赋值渲染
			console.log(ops)
			this.placeholderText=ops.placeholderText;
		},
		methods: {	
			conInput(){
